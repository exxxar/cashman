...
{
    public function uploadProducts($id, Request $request)
    {
        $company = Company::find($id);
        $group = $company->upload_vk_url;
        $number = strrchr($group, '/');
        $name = mb_substr($number, 1);
        $auth = new Auth('7883909', 'B95nspocgcljhxTATjTN', 'http://127.0.0.1:8000/auth/vkontakte/callback', ['group', 'market']);
        ...
        foreach ($response1["response"]["items"] as $item) {
            ...
            foreach ($response2["response"]["items"] as $item2) {
                ...
                if (is_null(Product::where('title', $item2['title'])->first())) {
