...
class CashBackController extends Controller
{
    public function debitingCashback(Request $request)
    {
        ...
        $this->debitingMultiLevelCashback($request->user, $request->company, $request->admin, $request->cashback, $request->sum, $request->description);
        ...
        if ($friends1 != null) {
            ...
            if (!is_null($cashback_percent_level_1)) {
                foreach ($friends1 as $friend1) {
                    ...
                    $this->debitingMultiLevelCashback($friend1->parent_id, $request->company, $request->admin, $cashback1, $request->sum, 'Начисление кэшбека 1 уровня от суммы чека вашего друга по компании');
                    ...
                    if ($friends2 != null) {
                        if (!is_null($cashback_percent_level_2)) {
                            foreach ($friends2 as $friend2) {
                                ...
                                $this->debitingMultiLevelCashback($friend2->parent_id, $request->company, $request->admin, $cashback2, $request->sum, 'Начисление кэшбека 2 уровня от суммы чека вашего друга по компании');
                            }
                        }
                    }
                }
    ...
    public function debitingMultiLevelCashback($user, $company, $admin, $cashback, $sum, $description)
    {
